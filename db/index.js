...
// SELECT s.score, p.player_name FROM scores AS s JOIN players AS p ON s.pid=p.id
// WHERE s.sent_id=${id} ORDER BY CAST(sc.score AS unsigned) LIMIT 10;
  ...
  connection.query(
    ...
    (err, results) => {
      if (err) {
        ...
        challenge.scores = [];
        connection.query(
          `SELECT s.score, p.player_name FROM scores AS s JOIN players AS p ON s.pid=p.id WHERE s.sent_id=${id} ORDER BY CAST(s.score AS unsigned) LIMIT 10;`,
          (scerr, scores) => {
            if (scerr) {
...
const insertTime = (score, callback) => {
  connection.query(`INSERT IGNORE INTO players (player_name) VALUES ("${score.player_name}");`, (perr) => {
    if (perr) {
      ...
      connection.query(`INSERT INTO scores (score, pid, sent_id) SELECT "${score.score}" AS score, id as pid, ${score.sent_id} AS sent_id FROM players WHERE player_name="${score.player_name}";`, (serr) => {
        if (serr) {
          callback(serr);
        } else {
          callback(null);
